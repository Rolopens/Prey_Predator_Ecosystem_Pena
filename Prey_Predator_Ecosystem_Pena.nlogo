...
breed [mice mouse]
turtles-own [
  energy ;; amount of energy a cat/mouse has
]
patches-own[
  food ;; if the current patch is food (1, 2, 3) ~ source of food
  growth ;; how long it will take for food patches to grow
]
...
to setup
  ...
  create-cats initial-cat-population [
    ...
    set color orange
    set size 1.5
    set energy random initial-cat-energy
    setxy random-xcor random-ycor
  ]
  create-mice initial-mice-population [
    set shape "mouse side"
    set color gray
    set size 1.5
    set energy random initial-rat-energy
    setxy random-xcor random-ycor
  ]
  grow-food
  reset-ticks
...
to grow-food
  ask patches with [pxcor >= -14 and pxcor <= -4 and pycor >= -14 and pycor <= -3][
    set pcolor yellow
    set food 1
  ]
  ask patches with [pxcor >= -14 and pxcor <= -4 and pycor >= 4 and pycor <= 14][
    set pcolor yellow
    set food 1
  ]
  ask patches with [pxcor >= 4 and pxcor <= 14 and pycor >= -5 and pycor <= 5][
    set pcolor yellow
    set food 1
  ]
...
to regrow-food
  ask patches with[ food = 1 and growth = 0][
    set pcolor yellow
  ]
  ask patches with [food = 1][
    set growth (growth - 1)
  ]
end

to reproduce-cats
  if random-float 100 < cat-reproduce [
    set energy (energy / 2)
...
to reproduce-mice
  if random-float 100 < mice-reproduce [
    set energy (energy / 2)
    hatch 1 [ rt random-float 360 fd 1 ]
  ]
end
...
to catch-mice
  ...
  if prey != nobody
    [ ask prey [ die ]
      set energy energy + energy-from-mice ]
end

to fix-energy
  ask turtles with [energy > max-energy][
...
to move
  ifelse coinflip? [right 30] [left 30]
  forward 1
  set energy (energy - 1)
end
...
to eat-food
  if pcolor = yellow
    [
      set pcolor black
      set growth food-growth
      set energy energy-from-food
    ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"Cats" 1.0 0 -955883 true "" "plot count cats"
"Mice" 1.0 0 -7500403 true "" "plot count mice"
...
"" ""
PENS
"default" 1.0 0 -1184463 true "" "plot count patches with [pcolor = yellow]"
